...
var AirbnbHomePage = {
	...
	init : function(a) {
		AirbnbHomePage.opts = a || {};
		AirbnbHomePage.initLocationBar();
		AirbnbHomePage.initSubmitListener();
		AirbnbHomePage.initCalendars()
	},
	initSlideshow : function() {$("#slideshow_container").cycle({
			fx : "fade",
			timeout : 5000,
			delay : -2000,
			prev : "#ss_button_prev",
			next : "#ss_button_next",
			fastOnEvent : 150,
			before : AirbnbHomePage.onBefore
		});
		$("#ss_button_pause_play").click(function(b) {
			var a = $(this);
			if(a.hasClass("ss_button_pause")) {a.removeClass("ss_button_pause").addClass("ss_button_play");
				$("#slideshow_container").cycle("pause")
			} else {a.removeClass("ss_button_play").addClass("ss_button_pause");
				$("#slideshow_container").cycle("next").cycle("resume")
			}b.preventDefault()
		});
		$("#slideshow").hover(function() {$("#slideshow_controls").show()
		}, function(a) {
			var b = $(a.relatedTarget);
			if($.inArray("slideshow", b.parents()) === -1) {$("#slideshow_controls").hide()
			}
		});
		if(Airbnb.Utils.usingIosDevice() === false) {HomePageSlideshow.enableKeypressListener()
		}
	},
	initLocationBar : function() {$.each($(".inner_text"), function(c, d) {
			var a = $(d).next("input");
			var b = a.val();
			a.attr("defaultValue", d.innerHTML);
			a.val(d.innerHTML);
			if(b.length !== 0) {a.val(b);
				a.addClass("active")
			...
			a.bind("blur", function() {
				if($(a).val() === "") {$(a).removeClass("active");
					$(a).val(a.attr("defaultValue"))
				} else {$(a).removeClass("error")
				}
			});
			AirbnbHomePage.fields_to_clear_on_submit.push(a);
			$(d).remove()
		});
		$("#location_label").show();
		if(AirbnbHomePage.opts.location) {$("#location").val(AirbnbHomePage.opts.location).addClass("active")
		}
	},
	initSubmitListener : function() {$("#search_form").submit(function() {
			if(AirbnbHomePage.checkInputs() === true) {
				if($.browser.msie && $.browser.version == "6.0") {
					window.location = [base_url+"search?location=",$("#location").val(),"&checkin=",$("#checkin").val(),"&checkout=",$("#checkout").val()].join("");
					return false
				}
				return true
			} else {
	...
	initCalendars : function() {
		...
		var c = $("#checkin");
		var d = $("#checkout");
		try {$.datepicker.parseDate(b, c.val());
	...
	locationIsBlank : function() {
		return (!$("#location").val() || (AirbnbHomePage.defaultSearchValue == $("#location").val()))
	},
	checkInputs : function() {
		if(AirbnbHomePage.locationIsBlank() === true) {
			var a = $("#enter_location_error_message");
			if(a.css("display") == "none") {a.show()
			} else {$("#enter_location_error_message").effect("pulsate", {
					times : 1
				}, 300)
			}
			return false
...
var HomePageSlideshow = {
		...
		if(HomePageSlideshow.keypressListenerEnabled === false) {
			...
			$(document.documentElement).bind(a, function(c) {
				if(HomePageSlideshow.freezeSlideshowControls === false && HomePageSlideshow.galleriaLoaded() === true) {
					...
					if(b == $.ui.keyCode.LEFT) {$("#slideshow_container").cycle("prev");
						HomePageSlideshow.freezeSlideshowControls = true
					} else {
						if(b == $.ui.keyCode.RIGHT) {$("#slideshow_container").cycle("next");
							HomePageSlideshow.freezeSlideshowControls = true
						}
	...
	stopPropagationOnInputs : function(a) {$("input").bind(a, function(c) {
			var b = c.keyCode || c.which;
			if(b == $.ui.keyCode.LEFT || b == $.ui.keyCode.RIGHT) {c.stopPropagation()
			}
		})
	}
};
(function(i) {
	...
	i.fn.cycle = function(s, r) {
		...
		if(this.length === 0 && s != "stop") {
			if(!i.isReady && t.s) {f("DOM not ready, queuing slideshow");
	...
	function b(r, s) {
		if(!i.support.opacity && s.cleartype && r.style.filter) {
			try {r.style.removeAttribute("filter")
			} catch(t) {
			}
		}
	}
	...
	function q(r) {
		if(r.next) {i(r.next).unbind(r.prevNextEvent)
		}
	...
	function k(z, L, v, u, F) {
		...
		if(D.random) {
			D.randomMap = [];
			for(var J = 0; J < v.length; J++) {D.randomMap.push(J)
		...
		L.css({position:"absolute",top:0,left:0}).hide().each(function(w) {
			var N;
			if(D.backwards) {
		...
		if(E) {
			...
			for(var H = 0; H < v.length; H++) {
				var r = i(v[H]), M = r[0], B = r.outerWidth(), K = r.outerHeight();
				if(!B) {
		...
		L.each(function() {
			...
			if(P.is("img")) {
				...
				if(N || Q || O || w) {
					if(F.s && D.requeueOnImageNotLoaded && ++u.requeueAttempts < 100) {f(u.requeueAttempts, " - img slide not loaded, requeuing slideshow: ", this.src, this.cycleW, this.cycleH);
		...
		if(!D.multiFx) {
			...
			if(i.isFunction(I)) {I(z, L, D)
				...
				if(D.fx != "custom" && !D.multiFx) {f("unknown transition: " + D.fx, "; slideshow terminating");
					return false
				}
			}
		}
		...
		if(D.next) {i(D.next).bind(D.prevNextEvent, function() {
				return o(D, D.rev ? -1 : 1)
			})
		}
		if(D.prev) {i(D.prev).bind(D.prevNextEvent, function() {
				return o(D, D.rev ? 1 : -1)
			})
		}
	...
	function e(y, r, x, A) {
		if(x && r.busy && r.manualTrump) {a("manualTrump in go(), stopping active transition");
			i(y).stop(true, true);
			r.busy = false
		}
		...
		if((x || !v.cyclePause) && (r.nextSlide != r.currSlide)) {
			...
			if(r.before.length) {i.each(r.before, function(D, E) {
					if(v.cycleStop != r.stopCount) {
						return
					}E.apply(B, [C, B, r, A])
				})
			}
			var t = function() {i.each(r.after, function(D, E) {
					if(v.cycleStop != r.stopCount) {
						return
					}E.apply(B, [C, B, r, A])
				})
			...
			if(r.fxFn) {r.fxFn(C, B, r, t, A, x && r.fastOnEvent)
				...
				if(i.isFunction(i.fn.cycle[r.fx])) {i.fn.cycle[r.fx](C, B, r, t, A, x && r.fastOnEvent)
		...
		if(s > 0) {
			v.cycleTimeout = setTimeout(function() {e(y, r, 0, (!r.rev && !r.backwards))
			}, s)
		}
	}
	...
	i.fn.cycle.createPagerAnchor = function(v, w, t, u, x) {
		...
		if(!/^click/.test(x.pagerEvent) && !x.allowPagerClickBubble) {y.bind("click.cycle", function() {
				return false
			})
		}
		if(x.pauseOnPagerHover) {y.hover(function() {
				x.$cont[0].cyclePause++
			}, function() {
				x.$cont[0].cyclePause--
			})
	...
	i.fn.cycle.hopsFromLast = function(u, t) {
		var s, r = u.lastSlide, v = u.currSlide;
		if(t) {
	...
	i.fn.cycle.custom = function(D, x, r, u, w, s) {
		...
		C.animate(r.animOut, B, A, function() {
			if(r.cssAfter) {C.css(r.cssAfter)
			}
			if(!r.sync) {z()
			}
		});
	...
	i.fn.cycle.transitions = {
		fade : function(s, t, r) {t.not(":eq("+r.currSlide+")").css("opacity", 0);
