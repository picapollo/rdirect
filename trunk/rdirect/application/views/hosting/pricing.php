...
<option value="true" selected="selected">any stay of 7 or more days</option></select> 
    ...
    <table class="season"> 
      <tr> 
      <?php foreach($weekly as $season)
      	echo "<td class=\"label\">{$season->label}</td>\n";
      ?>
      </tr> 
...
<?php		foreach($weekly as $season):
				echo '<td> <table class="weekly">'."\n";
					foreach($season->data as $week): ?>
                        <tr> 
                          <td class="dates" valign="middle"><?=$week->label?></td> 
                          <td> 
                            <span class="currency_symbol"><?=$room->currency_symbol?></span> 
                            <input id="weekly_price_<?=$week->date?>" name="<?=$week->date?>" type="text" value="" /> 
                          </td> 
                        </tr> 				
			<?php 	endforeach;
      			echo '</table></td> ';
