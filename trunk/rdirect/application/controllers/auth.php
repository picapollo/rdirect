...
class Auth extends MY_Controller
{
	...
	function login()
	{
		...
		{									
			redirect('home/dashboard');		// logged in
		} 
		else 
		{		
			$this->form_validation->set_rules('email', 'Login', 'trim|required|xss_clean');
			...
			if ($this->form_validation->run()) {								// validation ok
				...
				{	
					// success
					$loc_cur = $this->users_model->get_user_locale_and_currency($this->tank_auth->get_user_id());
	...
	function send_again()
	{
		...
		$this->data['top_menu']['notice'] = array();
		...
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->change_email(
					...
					$errors = $this->tank_auth->get_error_message();
					foreach ($errors as $k => $v)	$this->data['top_menu']['notice'][$k] = $this->lang->line($v);
				}
			}
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->forgot_password(
					...
					$this->_send_email('forgot_password', $this->data['email'], $data);
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_send_email('reset_password', $this->data['email'], $data);
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($this->data = $this->tank_auth->set_new_email(
					...
					$this->_show_message(sprintf($this->lang->line('auth_message_new_email_sent'), $this->data['new_email']));
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->delete_user(
					...
					$errors = $this->tank_auth->get_error_message();
					foreach ($errors as $k => $v)	$this->data['notices']['notice'][$k] = $this->lang->line($v);
				}
			}
