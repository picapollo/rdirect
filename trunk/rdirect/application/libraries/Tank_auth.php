...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('tank_auth/users_tank_auth');
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users_tank_auth->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							$this->ci->users_tank_auth->update_login_info(
									$user->id,
	...
	{
		if ((strlen($username) > 0) AND !$this->ci->users_tank_auth->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');

		} elseif (!$this->ci->users_tank_auth->is_email_available($email)) {
			...
			if (!is_null($res = $this->ci->users_tank_auth->create_user($data, !$email_activation))) {
	...
	{
		return ((strlen($username) > 0) AND $this->ci->users_tank_auth->is_username_available($username));
	}
	...
	{
		return ((strlen($email) > 0) AND $this->ci->users_tank_auth->is_email_available($email));
	}
	...
	function change_email($email)
	{
		...
		if (!is_null($user = $this->ci->users_tank_auth->get_user_by_id($user_id, FALSE))) {
			...
			if (strtolower($user->email) == strtolower($email)) {		// leave activation key as is
				...
				$data['new_email_key'] = md5(rand().microtime());
				$this->ci->users_tank_auth->set_new_email($user_id, $email, $data['new_email_key'], FALSE);
				return $data;
	...
	{
		$this->ci->users_tank_auth->purge_na($this->ci->config->item('email_activation_expire', 'tank_auth'));

		if ((strlen($user_id) > 0) AND (strlen($activation_key) > 0)) {
			return $this->ci->users_tank_auth->activate_user($user_id, $activation_key, $activate_by_email);
		}
		return FALSE;
	}
	...
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users_tank_auth->get_user_by_login($login))) {
				...
				$this->ci->users_tank_auth->set_password_key($user->id, $data['new_pass_key']);
				return $data;
	...
	function can_reset_password($user_id, $new_pass_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0)) {
			return $this->ci->users_tank_auth->can_reset_password(
				$user_id,
				$new_pass_key,
	...
	function reset_password($user_id, $new_pass_key, $new_password)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users_tank_auth->get_user_by_id($user_id, TRUE))) {
				...
				if ($this->ci->users_tank_auth->reset_password(
						$user_id,
						$hashed_password,
						$new_pass_key,
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users_tank_auth->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($old_pass, $user->password)) {			// success
				...
				$this->ci->users_tank_auth->change_password($user_id, $hashed_password);
				return TRUE;
	...
	function set_new_email($new_email, $password)
	{
		...
		if (!is_null($user = $this->ci->users_tank_auth->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				} elseif ($this->ci->users_tank_auth->is_email_available($new_email)) {
					$data['new_email_key'] = md5(rand().microtime());
					$this->ci->users_tank_auth->set_new_email($user_id, $new_email, $data['new_email_key'], TRUE);
					return $data;
	...
	function activate_new_email($user_id, $new_email_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_email_key) > 0)) {
			return $this->ci->users_tank_auth->activate_new_email(
					$user_id,
					$new_email_key);
		}
		return FALSE;
	...
	function delete_user($password)
	{
		...
		if (!is_null($user = $this->ci->users_tank_auth->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				$this->ci->users_tank_auth->delete_user($user_id);
				$this->logout();
				return TRUE;
